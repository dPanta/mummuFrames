...
if [[ ! -d "$FONT_DIR" ]]; then
  echo "Fonts directory not found: $FONT_DIR" >&2
  exit 1
fi
...
mapfile -t font_files < <(find "$FONT_DIR" -maxdepth 2 -type f \( -iname '*.ttf' -o -iname '*.otf' \) | sort)

{
  echo "local _, ns = ..."
  echo
  echo "-- Auto-generated by scripts/generate_font_catalog.sh."
  echo "-- Re-run this script after adding/removing files in Fonts/."
  echo "ns.FontCatalog = {"
  echo "    list = {"
